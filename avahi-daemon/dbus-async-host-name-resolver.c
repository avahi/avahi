...
void avahi_dbus_async_host_name_resolver_repeat_items(AsyncHostNameResolverInfo *i) {
	...
	if(i->host_name_resolver)
		avahi_s_host_name_resolver_repeat_items(i->host_name_resolver);
}
...
DBusHandlerResult avahi_dbus_msg_async_host_name_resolver_impl(DBusConnection *c, DBusMessage *m, void *userdata) {
    ...
    if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_HOST_NAME_RESOLVER, "RepeatItems")) {

        if (!dbus_message_get_args(m, &error, DBUS_TYPE_INVALID)) {
            avahi_log_warn("Error parsing HostNameResolver::RepeatItems message");
            goto fail;
        }

        avahi_dbus_async_host_name_resolver_repeat_items(i);
        return avahi_dbus_respond_ok(c, m);
    }
