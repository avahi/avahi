...
static int get_default_response(GtkDialog *dlg) {
    ...
    if (ret == GTK_RESPONSE_NONE) {
        ...
        while (t) {
            ...
            if (ret == GTK_RESPONSE_ACCEPT || ret == GTK_RESPONSE_OK || ret == GTK_RESPONSE_YES || ret == GTK_RESPONSE_APPLY)
                break;
...
static void aui_service_dialog_class_init(AuiServiceDialogClass *klass) {
    ...
    g_object_class_install_property(object_class,
                                    PROP_BROWSE_SERVICE_TYPES,
                                    g_param_spec_pointer("browse_service_types",
                                                         _("Browse Service Types"),
                                                         _("A NULL terminated list of service types to browse for"),
                                                         G_PARAM_READABLE | G_PARAM_WRITABLE));
    g_object_class_install_property(object_class,
                                    PROP_DOMAIN,
                                    g_param_spec_string("domain",
                                                        _("Domain"),
                                                        _("The domain to browse in, or NULL for the default domain"),
                                                        NULL,
                                                        G_PARAM_READABLE | G_PARAM_WRITABLE));
    g_object_class_install_property(object_class,
                                    PROP_SERVICE_TYPE,
                                    g_param_spec_string("service_type",
                                                        _("Service Type"),
                                                        _("The service type of the selected service"),
                                                        NULL,
                                                        G_PARAM_READABLE | G_PARAM_WRITABLE));
    g_object_class_install_property(object_class,
                                    PROP_SERVICE_NAME,
                                    g_param_spec_string("service_name",
                                                        _("Service Name"),
                                                        _("The service name of the selected service"),
                                                        NULL,
                                                        G_PARAM_READABLE | G_PARAM_WRITABLE));
    g_object_class_install_property(
        object_class,
        PROP_ADDRESS,
        g_param_spec_pointer("address", _("Address"), _("The address of the resolved service"), G_PARAM_READABLE));
    g_object_class_install_property(
        object_class,
        PROP_PORT,
        g_param_spec_uint("port", _("Port"), _("The IP port number of the resolved service"), 0, 0xFFFF, 0, G_PARAM_READABLE));
    g_object_class_install_property(
        object_class,
        PROP_HOST_NAME,
        g_param_spec_string("host_name", _("Host Name"), _("The host name of the resolved service"), NULL, G_PARAM_READABLE));
    g_object_class_install_property(
        object_class,
        PROP_TXT_DATA,
        g_param_spec_pointer("txt_data", _("TXT Data"), _("The TXT data of the resolved service"), G_PARAM_READABLE));
    g_object_class_install_property(object_class,
                                    PROP_RESOLVE_SERVICE,
                                    g_param_spec_boolean("resolve_service",
                                                         _("Resolve Service"),
                                                         _("Resolve the selected service automatically before returning"),
                                                         TRUE,
                                                         G_PARAM_READABLE | G_PARAM_WRITABLE));
    g_object_class_install_property(
        object_class,
        PROP_RESOLVE_HOST_NAME,
        g_param_spec_boolean("resolve_host_name",
                             _("Resolve Service Host Name"),
                             _("Resolve the host name of the selected service automatically before returning"),
                             TRUE,
                             G_PARAM_READABLE | G_PARAM_WRITABLE));
    g_object_class_install_property(object_class,
                                    PROP_ADDRESS_FAMILY,
                                    g_param_spec_int("address_family",
                                                     _("Address family"),
                                                     _("The address family for host name resolution"),
                                                     AVAHI_PROTO_UNSPEC,
                                                     AVAHI_PROTO_INET6,
                                                     AVAHI_PROTO_UNSPEC,
                                                     G_PARAM_READABLE | G_PARAM_WRITABLE));
...
GtkWidget *aui_service_dialog_new_valist(const gchar *title, GtkWindow *parent, const gchar *first_button_text,
    ...
    GtkWidget *w = (GtkWidget *)g_object_new(AUI_TYPE_SERVICE_DIALOG,
...
static void resolve_callback(AvahiServiceResolver *r G_GNUC_UNUSED, AvahiIfIndex interface G_GNUC_UNUSED,
                             AvahiProtocol protocol G_GNUC_UNUSED, AvahiResolverEvent event, const char *name, const char *type,
                             const char *domain, const char *host_name, const AvahiAddress *a, uint16_t port,
                             AvahiStringList *txt, AvahiLookupResultFlags flags G_GNUC_UNUSED, void *userdata) {
...
static void browse_callback(AvahiServiceBrowser *b G_GNUC_UNUSED, AvahiIfIndex interface, AvahiProtocol protocol,
    ...
    case AVAHI_BROWSER_NEW: {
        ...
        gtk_list_store_set(d->priv->service_list_store,
                           &iter,
                           SERVICE_COLUMN_IFACE,
                           interface,
                           SERVICE_COLUMN_PROTO,
        ...
        if (!gtk_tree_selection_get_selected(selection, NULL, NULL)) {
            ...
            if (!d->priv->service_type || !d->priv->service_name ||
    ...
    case AVAHI_BROWSER_REMOVE: {
        ...
        while (valid) {
            ...
            gtk_tree_model_get(GTK_TREE_MODEL(d->priv->service_list_store),
                               &iter,
                               SERVICE_COLUMN_IFACE,
                               &_interface,
                               SERVICE_COLUMN_PROTO,
            ...
            found = _interface == interface && _protocol == protocol && strcasecmp(_name, name) == 0 &&
                    avahi_domain_equal(_type, type);
            g_free(_name);
...
static void domain_browse_callback(AvahiDomainBrowser *b G_GNUC_UNUSED, AvahiIfIndex interface G_GNUC_UNUSED,
                                   AvahiProtocol protocol G_GNUC_UNUSED, AvahiBrowserEvent event, const char *name,
                                   AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, void *userdata) {
    ...
    case AVAHI_BROWSER_NEW: {
        ...
        while (valid) {
            ...
            gtk_tree_model_get(
                GTK_TREE_MODEL(d->priv->domain_list_store), &iter, DOMAIN_COLUMN_NAME, &_name, DOMAIN_COLUMN_REF, &ref, -1);
        ...
        if (found)
            ...
            gtk_list_store_set(d->priv->domain_list_store, &iter, DOMAIN_COLUMN_NAME, name, DOMAIN_COLUMN_REF, 1, -1);
        }
...
static gboolean start_callback(gpointer data) {
    ...
    for (st = d->priv->browse_service_types, sb = d->priv->browsers; *st; st++, sb++) {
        ...
        if (!(*sb = avahi_service_browser_new(
                  d->priv->client, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, *st, d->priv->domain, 0, browse_callback, d))) {
            GtkWidget *m = gtk_message_dialog_new(GTK_WINDOW(d),
                                                  GTK_DIALOG_DESTROY_WITH_PARENT,
                                                  GTK_MESSAGE_ERROR,
                                                  GTK_BUTTONS_CLOSE,
...
static void service_row_activated_callback(GtkTreeView *tree_view G_GNUC_UNUSED, GtkTreePath *path G_GNUC_UNUSED,
                                           GtkTreeViewColumn *column G_GNUC_UNUSED, gpointer user_data) {
...
static void response_callback(GtkDialog *dialog, gint response, gpointer user_data) {
    ...
    if ((response == GTK_RESPONSE_ACCEPT || response == GTK_RESPONSE_OK || response == GTK_RESPONSE_YES ||
        ...
        g_return_if_fail(gtk_tree_selection_get_selected(
            gtk_tree_view_get_selection(GTK_TREE_VIEW(d->priv->service_tree_view)), NULL, &iter));
        ...
        if (!(d->priv->resolver = avahi_service_resolver_new(d->priv->client,
                                                             interface,
                                                             protocol,
                                                             name,
                                                             type,
                                                             d->priv->domain,
                                                             d->priv->address_family,
                                                             !d->priv->resolve_host_name ? AVAHI_LOOKUP_NO_ADDRESS : 0,
                                                             resolve_callback,
                                                             d))) {
...
static void domain_row_activated_callback(GtkTreeView *tree_view G_GNUC_UNUSED, GtkTreePath *path G_GNUC_UNUSED,
                                          GtkTreeViewColumn *column G_GNUC_UNUSED, gpointer user_data) {
    AuiServiceDialog *d = AUI_SERVICE_DIALOG(user_data);
...
static void domain_selection_changed_callback(GtkTreeSelection *selection G_GNUC_UNUSED, gpointer user_data) {
    ...
    g_return_if_fail(
        gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(d->priv->domain_tree_view)), NULL, &iter));

    gtk_tree_model_get(GTK_TREE_MODEL(d->priv->domain_list_store), &iter, DOMAIN_COLUMN_NAME, &name, -1);
...
static void domain_entry_changed_callback(GtkEditable *editable G_GNUC_UNUSED, gpointer user_data) {
    ...
    gtk_widget_set_sensitive(d->priv->domain_ok_button,
                             is_valid_domain_suffix(gtk_entry_get_text(GTK_ENTRY(d->priv->domain_entry))));
}
...
static void domain_button_clicked(GtkButton *button G_GNUC_UNUSED, gpointer user_data) {
    ...
    if (!(p->domain_browser = avahi_domain_browser_new(p->client,
                                                       AVAHI_IF_UNSPEC,
                                                       AVAHI_PROTO_UNSPEC,
                                                       NULL,
...
static void aui_service_dialog_init(AuiServiceDialog *d) {
    ...
    p->service_list_store = gtk_list_store_new(
        N_SERVICE_COLUMNS, G_TYPE_INT, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
