...
static void record_browser_callback(AvahiSRecordBrowser *rr, AvahiIfIndex interface, AvahiProtocol protocol,
    ...
    case AVAHI_BROWSER_NEW: {
            ...
            if (!r->srv_record) {
                ...
                if (!(r->user_flags & AVAHI_LOOKUP_NO_ADDRESS)) {
                    ...
                    if (r->address_protocol == AVAHI_PROTO_INET || r->address_protocol == AVAHI_PROTO_UNSPEC) {
                        ...
                        r->record_browser_a =
                            avahi_s_record_browser_prepare(r->server,
                                                           r->interface,
                                                           r->protocol,
                                                           k,
                                                           r->user_flags & ~(AVAHI_LOOKUP_NO_TXT | AVAHI_LOOKUP_NO_ADDRESS),
        ...
        if (changed && r->srv_record && (r->txt_record || (r->user_flags & AVAHI_LOOKUP_NO_TXT)) &&
...
AvahiSServiceResolver *avahi_s_service_resolver_prepare(AvahiServer *server, AvahiIfIndex interface, AvahiProtocol protocol,
    ...
    r->record_browser_srv = avahi_s_record_browser_prepare(
        server, interface, protocol, k, flags & ~(AVAHI_LOOKUP_NO_TXT | AVAHI_LOOKUP_NO_ADDRESS), record_browser_callback, r);
    avahi_key_unref(k);
    ...
    if (!(flags & AVAHI_LOOKUP_NO_TXT)) {
        ...
        r->record_browser_txt = avahi_s_record_browser_prepare(server,
                                                               interface,
                                                               protocol,
                                                               k,
...
AvahiSServiceResolver *avahi_s_service_resolver_new(AvahiServer *server, AvahiIfIndex interface, AvahiProtocol protocol,
                                                    const char *name, const char *type, const char *domain,
                                                    AvahiProtocol aprotocol, AvahiLookupFlags flags,
                                                    AvahiSServiceResolverCallback callback, void *userdata) {
