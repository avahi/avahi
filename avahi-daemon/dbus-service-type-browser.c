...
void avahi_dbus_service_type_browser_repeat_items(ServiceTypeBrowserInfo *i) {
    ...
    if(i->service_type_browser)
        avahi_s_service_type_browser_repeat_items(i->service_type_browser);
}
...
DBusHandlerResult avahi_dbus_msg_service_type_browser_impl(DBusConnection *c, DBusMessage *m, void *userdata) {
    ...
    if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVICE_TYPE_BROWSER, "Free")) {
     ...
     if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVICE_TYPE_BROWSER, "RepeatItems")) {

        if (!dbus_message_get_args(m, &error, DBUS_TYPE_INVALID)) {
            avahi_log_warn("Error parsing ServiceTypeBrowser::RepeatItems message");
            goto fail;
        }

        avahi_dbus_service_type_browser_repeat_items(i);
        return avahi_dbus_respond_ok(c, m);
