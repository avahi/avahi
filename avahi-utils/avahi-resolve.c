...
static void host_name_resolver_callback(AvahiHostNameResolver *r, AVAHI_GCC_UNUSED AvahiIfIndex interface,
...
static void address_resolver_callback(AvahiAddressResolver *r, AVAHI_GCC_UNUSED AvahiIfIndex interface,
                                      AVAHI_GCC_UNUSED AvahiProtocol protocol, AvahiResolverEvent event, const AvahiAddress *a,
                                      const char *name, AVAHI_GCC_UNUSED AvahiLookupResultFlags flags,
                                      AVAHI_GCC_UNUSED void *userdata) {
...
static int parse_command_line(Config *c, const char *argv0, int argc, char *argv[]) {
    ...
    c->command = strstr(argv0, "address") ? COMMAND_RESOLVE_ADDRESS
                                          : (strstr(argv0, "host-name") ? COMMAND_RESOLVE_HOST_NAME : COMMAND_UNSPEC);
    ...
    case COMMAND_RESOLVE_ADDRESS: {
        ...
        for (i = optind; i < argc; i++) {
            ...
            if (config.command == COMMAND_RESOLVE_HOST_NAME) {
                ...
                if (!(avahi_host_name_resolver_new(client,
                                                   AVAHI_IF_UNSPEC,
                                                   AVAHI_PROTO_UNSPEC,
                                                   argv[i],
                                                   config.proto,
                ...
                if (!(avahi_address_resolver_new(
                        client, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, &a, 0, address_resolver_callback, NULL))) {
