...
AvahiSHostNameResolver *avahi_s_host_name_resolver_prepare(
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(server, aprotocol != AVAHI_PROTO_INET || server->config.use_ipv4, AVAHI_ERR_INVALID_PROTOCOL);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(server, aprotocol != AVAHI_PROTO_INET6 || server->config.use_ipv6, AVAHI_ERR_INVALID_PROTOCOL);
