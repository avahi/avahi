...
static int register_stuff(Config *config) {
    ...
    if (config->command == COMMAND_PUBLISH_ADDRESS) {
        ...
        if (avahi_entry_group_add_address(entry_group,
                                          AVAHI_IF_UNSPEC,
                                          AVAHI_PROTO_UNSPEC,
                                          config->no_reverse ? AVAHI_PUBLISH_NO_REVERSE : 0,
                                          config->name,
                                          &config->address) < 0) {
        ...
        if (avahi_entry_group_add_service_strlst(entry_group,
                                                 AVAHI_IF_UNSPEC,
                                                 AVAHI_PROTO_UNSPEC,
                                                 0,
                                                 config->name,
        ...
        for (i = config->subtypes; i; i = i->next)
            if (avahi_entry_group_add_service_subtype(entry_group,
                                                      AVAHI_IF_UNSPEC,
                                                      AVAHI_PROTO_UNSPEC,
                                                      0,
                                                      config->name,
...
static int parse_command_line(Config *c, const char *argv0, int argc, char *argv[]) {
    ...
    c->command = strstr(argv0, "address") ? COMMAND_PUBLISH_ADDRESS
                                          : (strstr(argv0, "service") ? COMMAND_PUBLISH_SERVICE : COMMAND_UNSPEC);
