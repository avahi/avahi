...
#define GA_SERVICE_RESOLVER_GET_PRIVATE(o)                                                                                     \
    (G_TYPE_INSTANCE_GET_PRIVATE((o), GA_TYPE_SERVICE_RESOLVER, GaServiceResolverPrivate))
...
static void ga_service_resolver_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec) {
...
static void ga_service_resolver_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec) {
    GaServiceResolver        *resolver = GA_SERVICE_RESOLVER(object);
    GaServiceResolverPrivate *priv = GA_SERVICE_RESOLVER_GET_PRIVATE(resolver);
...
static void ga_service_resolver_class_init(GaServiceResolverClass *ga_service_resolver_class) {
    GObjectClass *object_class = G_OBJECT_CLASS(ga_service_resolver_class);
    GParamSpec   *param_spec;

    g_type_class_add_private(ga_service_resolver_class, sizeof(GaServiceResolverPrivate));
    ...
    param_spec = g_param_spec_enum("protocol",
    ...
    param_spec = g_param_spec_enum("aprotocol",
                                   "Address protocol",
    ...
    param_spec = g_param_spec_int("interface",
                                  "interface index",
    ...
    param_spec = g_param_spec_string(
        "name", "service name", "name to resolve", NULL, G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_NAME, param_spec);

    param_spec = g_param_spec_string("type",
                                     "service type",
...
static void _avahi_service_resolver_cb(AVAHI_GCC_UNUSED AvahiServiceResolver *resolver, AvahiIfIndex interface,
    ...
    case AVAHI_RESOLVER_FOUND: {
        ...
        priv->address = *a;
        priv->port = port;
        g_signal_emit(self, signals[FOUND], 0, interface, protocol, name, type, domain, host_name, a, port, txt, flags);
        break;
    }
    case AVAHI_RESOLVER_FAILURE: {
        GError *error;
        int     aerrno = avahi_client_errno(priv->client->avahi_client);
        error = g_error_new(GA_ERROR, aerrno, "Resolving failed: %s", avahi_strerror(aerrno));
...
GaServiceResolver *ga_service_resolver_new(AvahiIfIndex interface, AvahiProtocol protocol, const gchar *name, const gchar *type,
                                           const gchar *domain, AvahiProtocol address_protocol, GaLookupFlags flags) {
