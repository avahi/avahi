...
static int rtm_dispatch(void) {
    ...
    di = malloc(sizeof(*di));
    if (di == NULL) {
        daemon_log(LOG_ERR, "malloc(%d): %s", sizeof(*di), strerror(errno));
        return (-1);
    }
    ...
    if (di->di_buf == NULL) {
        free(di);
        daemon_log(LOG_ERR, "calloc(%d): %s", MAX_RTMSG_SIZE, strerror(errno));
        return (-1);
    }
    ...
    for (;;) {
        ...
        if (len == -1) {
            if (errno == EWOULDBLOCK)
                ...
                daemon_log(LOG_ERR, "recvmsg(): %s", strerror(errno));
                retval = -1;
                break;
            }
        }
        ...
        if (rtm->rtm_version != RTM_VERSION) {
            daemon_log(LOG_ERR, "unknown routing socket message (version %d)\n", rtm->rtm_version);
...
static int rtm_dispatch_ifannounce(struct rtm_dispinfo *di) {
    ...
    daemon_log(LOG_DEBUG, "%s: IFANNOUNCE for ifindex %d", __func__, rtm->ifan.ifan_index);

    switch (rtm->ifan.ifan_what) {
    case IFAN_ARRIVAL:
        if (rtm->ifan.ifan_index == ifindex) {
            daemon_log(LOG_ERR, "RTM_IFANNOUNCE IFAN_ARRIVAL, for ifindex %d, which we already manage.", ifindex);
            return (-1);
        }
        break;
    case IFAN_DEPARTURE:
...
#define SKIPRTA(ifamsgp, rta, sa)                                                                                              \
    ...
    assert(ifam->ifam_type == RTM_NEWADDR || ifam->ifam_type == RTM_DELADDR);
    ...
    daemon_log(LOG_DEBUG,
               ...
               ifam->ifam_index,
               (ifam->ifam_index == ifindex) ? "ours" : "not ours");
    ...
    daemon_log(LOG_DEBUG,
               ...
               link_local ? "link local" : "routable");
