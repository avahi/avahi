...
static void watch_cb(evutil_socket_t fd, short what, void *arg) {
    AvahiWatch     *w = arg;
...
static int watch_add(AvahiWatch *w, int fd, AvahiWatchEvent events) {
...
static AvahiWatch *watch_new(const AvahiPoll *api, int fd, AvahiWatchEvent events, AvahiWatchCallback cb, void *userdata) {
...
static void timeout_cb(AVAHI_GCC_UNUSED evutil_socket_t fd, AVAHI_GCC_UNUSED short events, void *arg) {
    AvahiTimeout *t = arg;

    t->cb(t, t->userdata);
}

static int timeout_add(AvahiTimeout *t, const struct timeval *tv) {
...
static AvahiTimeout *timeout_new(const AvahiPoll *api, const struct timeval *tv, AvahiTimeoutCallback cb, void *userdata) {
    AvahiLibeventPoll *ep;
    AvahiTimeout      *t;
    int                ret;
