...
void avahi_interface_address_update_rrs(AvahiInterfaceAddress *a, int remove_rrs) {
    ...
    if (m->list_complete &&
        ...
        if (avahi_s_entry_group_is_empty(a->entry_group)) {
            ...
            if (avahi_server_add_address(m->server, a->entry_group, a->interface->hardware->index, p, AVAHI_PUBLISH_DEFAULT, NULL, &a->address) < 0) {
...
void avahi_hw_interface_update_rrs(AvahiHwInterface *hw, int remove_rrs) {
    ...
    if (m->list_complete &&
        ...
        if (avahi_s_entry_group_is_empty(hw->entry_group)) {
            ...
            if (avahi_server_add_service(m->server, hw->entry_group, hw->index, AVAHI_PROTO_UNSPEC, AVAHI_PUBLISH_DEFAULT, name, "_workstation._tcp", NULL, NULL, 9, NULL) < 0) {
