...
static void record_browser_callback(AvahiSRecordBrowser *r, AvahiIfIndex interface, AvahiProtocol protocol,
                                    AvahiBrowserEvent event, AvahiRecord *record, AVAHI_GCC_UNUSED AvahiLookupResultFlags flags,
    ...
    if (record) {
        avahi_log_debug("RB: record [%s] on %i.%i is %s",
...
static void create_entries(int new_name) {
    ...
    if (avahi_server_add_service(
            server, group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, service_name, "_http._tcp", NULL, NULL, 80, "foo", NULL) <
        0) {
    ...
    if (avahi_server_add_service(
            server, group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, service_name, "_ftp._tcp", NULL, NULL, 21, "foo", NULL) <
        0) {
    ...
    if (avahi_server_add_service(
            server, group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, service_name, "_webdav._tcp", NULL, NULL, 80, "foo", NULL) <
        0) {
        avahi_log_error("Failed to add WEBDAV service");
        goto fail;
    }

    if (avahi_server_add_dns_server_address(server,
                                            group,
                                            AVAHI_IF_UNSPEC,
                                            AVAHI_PROTO_UNSPEC,
                                            0,
                                            NULL,
                                            AVAHI_DNS_SERVER_RESOLVE,
...
static void ar_callback(AVAHI_GCC_UNUSED AvahiSAddressResolver *r, AvahiIfIndex iface, AvahiProtocol protocol,
                        AvahiResolverEvent event, const AvahiAddress *a, const char *hostname,
                        AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, AVAHI_GCC_UNUSED void *userdata) {
    char t[AVAHI_ADDRESS_STR_MAX];
    ...
    avahi_log_debug(
        "AR: (%i.%i) %s -> <%s> [%s]", iface, protocol, t, hostname ? hostname : "n/a", resolver_event_to_string(event));
}

static void db_callback(AVAHI_GCC_UNUSED AvahiSDomainBrowser *b, AvahiIfIndex iface, AvahiProtocol protocol,
...
static void stb_callback(AVAHI_GCC_UNUSED AvahiSServiceTypeBrowser *b, AvahiIfIndex iface, AvahiProtocol protocol,
                         AvahiBrowserEvent event, const char *service_type, const char *domain,
                         AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, AVAHI_GCC_UNUSED void *userdata) {

    avahi_log_debug("STB: (%i.%i) %s in <%s> [%s]",
                    iface,
                    protocol,
...
static void sb_callback(AVAHI_GCC_UNUSED AvahiSServiceBrowser *b, AvahiIfIndex iface, AvahiProtocol protocol,
                        AvahiBrowserEvent event, const char *name, const char *service_type, const char *domain,
                        AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, AVAHI_GCC_UNUSED void *userdata) {
    avahi_log_debug("SB: (%i.%i) <%s> as %s in <%s> [%s]",
                    iface,
                    protocol,
                    name ? name : "NULL",
                    service_type ? service_type : "NULL",
...
static void sr_callback(AVAHI_GCC_UNUSED AvahiSServiceResolver *r, AvahiIfIndex iface, AvahiProtocol protocol,
                        AvahiResolverEvent event, const char *name, const char *service_type, const char *domain_name,
                        const char *hostname, const AvahiAddress *a, uint16_t port, AvahiStringList *txt,
                        AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, AVAHI_GCC_UNUSED void *userdata) {

    if (event != AVAHI_RESOLVER_FOUND)
        avahi_log_debug("SR: (%i.%i) <%s> as %s in <%s> [%s]",
                        iface,
                        protocol,
                        name,
                        service_type,
        ...
        avahi_log_debug("SR: (%i.%i) <%s> as %s in <%s>: %s/%s:%i (%s) [%s]",
                        iface,
                        protocol,
                        name,
                        service_type,
...
static void dsb_callback(AVAHI_GCC_UNUSED AvahiSDNSServerBrowser *b, AvahiIfIndex iface, AvahiProtocol protocol,
                         AvahiBrowserEvent event, const char *hostname, const AvahiAddress *a, uint16_t port,
                         AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, AVAHI_GCC_UNUSED void *userdata) {
    ...
    avahi_log_debug(
        "DSB: (%i.%i): %s/%s:%i [%s]", iface, protocol, hostname ? hostname : "NULL", t, port, browser_event_to_string(event));
}
...
int main(AVAHI_GCC_UNUSED int argc, AVAHI_GCC_UNUSED char *argv[]) {
    ...
    hnr = avahi_s_host_name_resolver_prepare(
        server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, "cname.local", AVAHI_PROTO_UNSPEC, 0, hnr_callback, NULL);
    avahi_s_host_name_resolver_start(hnr);

    ar = avahi_s_address_resolver_prepare(server,
                                          AVAHI_IF_UNSPEC,
                                          AVAHI_PROTO_UNSPEC,
                                          avahi_address_parse("192.168.50.1", AVAHI_PROTO_INET, &a),
                                          0,
                                          ar_callback,
                                          NULL);
    avahi_s_address_resolver_start(ar);

    db = avahi_s_domain_browser_prepare(
        server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, NULL, AVAHI_DOMAIN_BROWSER_BROWSE, 0, db_callback, NULL);
    ...
    sr = avahi_s_service_resolver_prepare(server,
                                          AVAHI_IF_UNSPEC,
                                          AVAHI_PROTO_UNSPEC,
                                          "Ecstasy HTTP",
                                          "_http._tcp",
    ...
    dsb = avahi_s_dns_server_browser_prepare(server,
                                             AVAHI_IF_UNSPEC,
                                             AVAHI_PROTO_UNSPEC,
                                             "local",
                                             AVAHI_DNS_SERVER_RESOLVE,
