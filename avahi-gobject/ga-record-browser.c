...
static void ga_record_browser_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec) {
...
static void ga_record_browser_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec) {
    GaRecordBrowser        *browser = GA_RECORD_BROWSER(object);
    GaRecordBrowserPrivate *priv = GA_RECORD_BROWSER_GET_PRIVATE(browser);
...
static void ga_record_browser_class_init(GaRecordBrowserClass *ga_record_browser_class) {
    ...
    g_type_class_add_private(ga_record_browser_class, sizeof(GaRecordBrowserPrivate));
    ...
    signals[NEW] = g_signal_new("new-record",
    ...
    signals[REMOVED] = g_signal_new("removed-record",
                                    G_OBJECT_CLASS_TYPE(ga_record_browser_class),
                                    G_SIGNAL_RUN_LAST,
                                    0,
                                    NULL,
    ...
    signals[CACHE_EXHAUSTED] = g_signal_new("cache-exhausted",
                                            G_OBJECT_CLASS_TYPE(ga_record_browser_class),
                                            G_SIGNAL_RUN_LAST,
                                            0,
                                            NULL,
    ...
    param_spec = g_param_spec_uint("type",
                                   "record type",
    ...
    param_spec = g_param_spec_uint("class",
                                   "record class",
...
GaRecordBrowser *ga_record_browser_new(const gchar *name, guint16 type) {
    return ga_record_browser_new_full(AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, name, AVAHI_DNS_CLASS_IN, type, 0);
}

GaRecordBrowser *ga_record_browser_new_full(AvahiIfIndex interface, AvahiProtocol protocol, const gchar *name, guint16 clazz,
...
static void _avahi_record_browser_cb(AVAHI_GCC_UNUSED AvahiRecordBrowser *r, AvahiIfIndex interface, AvahiProtocol protocol,
                                     AvahiBrowserEvent event, const char *name, uint16_t clazz, uint16_t type,
                                     const void *rdata, size_t rdata_size, AvahiLookupResultFlags flags, void *userdata) {
    ...
    case AVAHI_BROWSER_REMOVE: {
        ...
        g_signal_emit(self, signals[signalid], 0, interface, protocol, name, clazz, type, rdata, rdata_size, flags);
        break;
    }
    ...
    case AVAHI_BROWSER_FAILURE: {
        ...
        error = g_error_new(GA_ERROR, aerrno, "Browsing failed: %s", avahi_strerror(aerrno));
...
gboolean ga_record_browser_attach(GaRecordBrowser *browser, GaClient *client, GError **error) {
    ...
    if (priv->browser == NULL) {
        if (error != NULL) {
            ...
            *error = g_error_new(GA_ERROR, aerrno, "Attaching record browser failed: %s", avahi_strerror(aerrno));
        }
        return FALSE;
    }
    return TRUE;
