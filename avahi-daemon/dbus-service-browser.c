...
void avahi_dbus_service_browser_repeat_items(ServiceBrowserInfo *i) {
    ...
    if(i->service_browser)
        avahi_s_service_browser_repeat_items(i->service_browser);
}
...
DBusHandlerResult avahi_dbus_msg_service_browser_impl(DBusConnection *c, DBusMessage *m, void *userdata) {
    ...
    if(dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVICE_BROWSER, "RepeatItems")) {

         if (!dbus_message_get_args(m, &error, DBUS_TYPE_INVALID)) {
            avahi_log_warn("Error parsing ServiceBrowser::RepeatItems message");
            goto fail;
        }

        avahi_dbus_service_browser_repeat_items(i);
        return avahi_dbus_respond_ok(c, m);
