...
static void ga_service_browser_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec) {
...
static void ga_service_browser_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec) {
    GaServiceBrowser        *browser = GA_SERVICE_BROWSER(object);
    GaServiceBrowserPrivate *priv = GA_SERVICE_BROWSER_GET_PRIVATE(browser);
...
static void ga_service_browser_class_init(GaServiceBrowserClass *ga_service_browser_class) {
    GObjectClass *object_class = G_OBJECT_CLASS(ga_service_browser_class);
    GParamSpec   *param_spec;

    g_type_class_add_private(ga_service_browser_class, sizeof(GaServiceBrowserPrivate));
    ...
    signals[NEW] = g_signal_new("new-service",
    ...
    signals[REMOVED] = g_signal_new("removed-service",
                                    G_OBJECT_CLASS_TYPE(ga_service_browser_class),
                                    G_SIGNAL_RUN_LAST,
                                    0,
                                    NULL,
...
static void _avahi_service_browser_cb(AvahiServiceBrowser *b, AvahiIfIndex interface, AvahiProtocol protocol,
    ...
    case AVAHI_BROWSER_REMOVE: {
        ...
        g_signal_emit(self, signals[signalid], 0, interface, protocol, name, type, domain, flags);
        break;
    }
...
GaServiceBrowser *ga_service_browser_new(const gchar *type) {
    return ga_service_browser_new_full(AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, type, NULL, 0);
}

GaServiceBrowser *ga_service_browser_new_full(AvahiIfIndex interface, AvahiProtocol protocol, const gchar *type, gchar *domain,
                                              GaLookupFlags flags) {
