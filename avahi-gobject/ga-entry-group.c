...
static void ga_entry_group_init(GaEntryGroup *obj) {
    ...
    priv->client = NULL;
    priv->group = NULL;
    priv->services = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, _free_service);
}

static void ga_entry_group_dispose(GObject *object);
static void ga_entry_group_finalize(GObject *object);

static void ga_entry_group_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec) {
...
static void _avahi_entry_group_cb(AvahiEntryGroup *g, AvahiEntryGroupState state, void *data) {
...
GaEntryGroup *ga_entry_group_new(void) { return g_object_new(GA_TYPE_ENTRY_GROUP, NULL); }
...
static void _set_entry(GHashTable *table, const guint8 *key, gsize ksize, const guint8 *value, gsize vsize) {
...
static GHashTable *_string_list_to_hash(AvahiStringList *list) {
    ...
    ret = g_hash_table_new_full(_entry_hash, _entry_equal, _free_entry, _free_entry);
    ...
    for (t = list; t != NULL; t = avahi_string_list_get_next(t)) {
        ...
        if (value == NULL) {
            ...
            _set_entry(ret, (const guint8 *)key, strlen(key), (const guint8 *)value, size);
        }
        avahi_free(key);
        avahi_free(value);
    }
...
static void _hash_to_string_list_foreach(gpointer key, gpointer value, gpointer data) {
    ...
    if (value != NULL) {
        *list = avahi_string_list_add_pair_arbitrary(*list, (gchar *)kentry->value, ventry->value, ventry->size);
    } else {
        *list = avahi_string_list_add_arbitrary(*list, kentry->value, kentry->size);
    }
}
...
GaEntryGroupService *ga_entry_group_add_service_strlist(GaEntryGroup *group, const gchar *name, const gchar *type, guint16 port,
                                                        GError **error, AvahiStringList *txt) {
    return ga_entry_group_add_service_full_strlist(
        group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, name, type, NULL, NULL, port, error, txt);
}

GaEntryGroupService *ga_entry_group_add_service_full_strlist(GaEntryGroup *group, AvahiIfIndex interface,
                                                             AvahiProtocol protocol, AvahiPublishFlags flags, const gchar *name,
                                                             const gchar *type, const gchar *domain, const gchar *host,
                                                             guint16 port, GError **error, AvahiStringList *txt) {
    GaEntryGroupPrivate        *priv = GA_ENTRY_GROUP_GET_PRIVATE(group);
    GaEntryGroupServicePrivate *service = NULL;
    int                         ret;

    ret = avahi_entry_group_add_service_strlst(priv->group, interface, protocol, flags, name, type, domain, host, port, txt);
    if (ret) {
        if (error != NULL) {
            *error = g_error_new(GA_ERROR, ret, "Adding service to group failed: %s", avahi_strerror(ret));
        }
        goto out;
    }
...
GaEntryGroupService *ga_entry_group_add_service(GaEntryGroup *group, const gchar *name, const gchar *type, guint16 port,
                                                GError **error, ...) {
...
GaEntryGroupService *ga_entry_group_add_service_full(GaEntryGroup *group, AvahiIfIndex interface, AvahiProtocol protocol,
    ...
    ret =
        ga_entry_group_add_service_full_strlist(group, interface, protocol, flags, name, type, domain, host, port, error, txt);
    avahi_string_list_free(txt);
    va_end(va);
    return ret;
}

gboolean ga_entry_group_add_record(GaEntryGroup *group, AvahiPublishFlags flags, const gchar *name, guint16 type, guint32 ttl,
                                   const void *rdata, gsize size, GError **error) {
    return ga_entry_group_add_record_full(
        group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, flags, name, AVAHI_DNS_CLASS_IN, type, ttl, rdata, size, error);
}

gboolean ga_entry_group_add_record_full(GaEntryGroup *group, AvahiIfIndex interface, AvahiProtocol protocol,
                                        AvahiPublishFlags flags, const gchar *name, guint16 clazz, guint16 type, guint32 ttl,
                                        const void *rdata, gsize size, GError **error) {
    int                  ret;
    GaEntryGroupPrivate *priv = GA_ENTRY_GROUP_GET_PRIVATE(group);
    g_assert(group != NULL && priv->group != NULL);

    ret = avahi_entry_group_add_record(priv->group, interface, protocol, flags, name, clazz, type, ttl, rdata, size);
    if (ret) {
        if (error != NULL) {
            *error = g_error_new(GA_ERROR, ret, "Setting raw record failed: %s", avahi_strerror(ret));
        }
        return FALSE;
    }
    return TRUE;
...
gboolean ga_entry_group_service_thaw(GaEntryGroupService *service, GError **error) {
    ...
    ret = avahi_entry_group_update_service_txt_strlst(GA_ENTRY_GROUP_GET_PRIVATE(priv->group)->group,
                                                      service->interface,
                                                      service->protocol,
                                                      service->flags,
                                                      service->name,
    ...
    if (ret) {
        if (error != NULL) {
            *error = g_error_new(GA_ERROR, ret, "Updating txt record failed: %s", avahi_strerror(ret));
        }
        result = FALSE;
    }
...
gboolean ga_entry_group_service_set(GaEntryGroupService *service, const gchar *key, const gchar *value, GError **error) {
    return ga_entry_group_service_set_arbitrary(service, key, (const guint8 *)value, strlen(value), error);
}

gboolean ga_entry_group_service_set_arbitrary(GaEntryGroupService *service, const gchar *key, const guint8 *value, gsize size,
                                              GError **error) {
    GaEntryGroupServicePrivate *priv = (GaEntryGroupServicePrivate *)service;
...
gboolean ga_entry_group_attach(GaEntryGroup *group, GaClient *client, GError **error) {
    ...
    priv->group = avahi_entry_group_new(client->avahi_client, _avahi_entry_group_cb, group);
    if (priv->group == NULL) {
        if (error != NULL) {
            int aerrno = avahi_client_errno(client->avahi_client);
            *error = g_error_new(GA_ERROR, aerrno, "Attaching group failed: %s", avahi_strerror(aerrno));
        }
        return FALSE;
    }
    return TRUE;
...
gboolean ga_entry_group_commit(GaEntryGroup *group, GError **error) {
    ...
    if (ret) {
        if (error != NULL) {
            *error = g_error_new(GA_ERROR, ret, "Committing group failed: %s", avahi_strerror(ret));
        }
        return FALSE;
    }
    return TRUE;
...
gboolean ga_entry_group_reset(GaEntryGroup *group, GError **error) {
    ...
    if (ret) {
        if (error != NULL) {
            *error = g_error_new(GA_ERROR, ret, "Resetting group failed: %s", avahi_strerror(ret));
        }
        return FALSE;
    }
    return TRUE;
