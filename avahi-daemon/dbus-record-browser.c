...
void avahi_dbus_record_browser_repeat_items(RecordBrowserInfo *i) {
    ...
    if(i->record_browser)
        avahi_s_record_browser_repeat_items(i->record_browser);
}
...
DBusHandlerResult avahi_dbus_msg_record_browser_impl(DBusConnection *c, DBusMessage *m, void *userdata) {
    ...
    if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_RECORD_BROWSER, "RepeatItems")) {

        if (!dbus_message_get_args(m, &error, DBUS_TYPE_INVALID)) {
            avahi_log_warn("Error parsing RecordBrowser::RepeatItems message");
            goto fail;
        }

        avahi_dbus_record_browser_repeat_items(i);
        return avahi_dbus_respond_ok(c, m);
