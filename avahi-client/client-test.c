...
static void avahi_domain_browser_callback(AvahiDomainBrowser *b, AvahiIfIndex interface, AvahiProtocol protocol,
...
static void avahi_service_resolver_callback(AvahiServiceResolver *r, AvahiIfIndex interface, AvahiProtocol protocol,
    ...
    printf("SERVICE-RESOLVER: Callback on ServiceResolver, interface (%d), protocol (%d), event (%d), name (%s), type (%s), "
           "domain (%s), host_name (%s), address (%s), port (%d), txtdata (%s), data(%s)\n",
           interface,
           protocol,
           event,
           name,
...
static void avahi_service_browser_callback(AvahiServiceBrowser *b, AvahiIfIndex interface, AvahiProtocol protocol,
                                           AvahiBrowserEvent event, const char *name, const char *type, const char *domain,
                                           AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, void *userdata) {

    AvahiServiceResolver *sr;

    printf("SERVICE-BROWSER: Callback on %p, interface (%d), protocol (%d), event (%d), name (%s), type (%s), domain (%s), "
           "data (%s)\n",
           (void *)b,
           interface,
           protocol,
    ...
    if (b && name) {
        sr = avahi_service_resolver_new(avahi_service_browser_get_client(b),
                                        interface,
                                        protocol,
                                        name,
                                        type,
...
static void avahi_service_type_browser_callback(AvahiServiceTypeBrowser *b, AvahiIfIndex interface, AvahiProtocol protocol,
                                                AvahiBrowserEvent event, const char *type, const char *domain,
                                                AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, void *userdata) {

    printf(
        "SERVICE-TYPE-BROWSER: Callback on %p, interface (%d), protocol (%d), event (%d), type (%s), domain (%s), data (%s)\n",
        (void *)b,
        interface,
        protocol,
        event,
...
static void avahi_address_resolver_callback(AVAHI_GCC_UNUSED AvahiAddressResolver *r, AvahiIfIndex interface,
                                            AvahiProtocol protocol, AvahiResolverEvent event, const AvahiAddress *address,
                                            const char *name, AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, void *userdata) {
    ...
    printf("ADDRESS-RESOLVER: Callback on AddressResolver, interface (%d), protocol (%d), even (%d), address (%s), name (%s), "
           "data(%s)\n",
           interface,
           protocol,
           event,
...
static void avahi_host_name_resolver_callback(AvahiHostNameResolver *r, AvahiIfIndex interface, AvahiProtocol protocol,
    ...
    avahi_address_snprint(addr, sizeof(addr), a);
    printf("HOST-NAME-RESOLVER: Callback on HostNameResolver, interface (%d), protocol (%d), event (%d), name (%s), address "
           "(%s), data (%s)\n",
           interface,
           protocol,
           event,
...
static void test_entry_group_reset(AVAHI_GCC_UNUSED AvahiTimeout *timeout, void *userdata) {
    ...
    avahi_entry_group_add_service(
        g, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, "Lathiat's Site", "_http._tcp", NULL, NULL, 80, "foo=bar2", NULL);
...
static void test_entry_group_update(AVAHI_GCC_UNUSED AvahiTimeout *timeout, void *userdata) {
    ...
    avahi_entry_group_update_service_txt(
        g, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, "Lathiat's Site", "_http._tcp", NULL, "foo=bar3", NULL);
}
...
int main(AVAHI_GCC_UNUSED int argc, AVAHI_GCC_UNUSED char *argv[]) {
    ...
    printf("Local service cookie: %u (Error Return: %s)\n",
           cookie,
           cookie != AVAHI_SERVICE_COOKIE_INVALID ? "OK" : avahi_strerror(avahi_client_errno(avahi)));
    ...
    printf(
        "%s\n",
        avahi_strerror(avahi_entry_group_add_service(
            group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, "Lathiat's Site", "_http._tcp", NULL, NULL, 80, "foo=bar", NULL)));
    printf("add_record: %d\n",
    ...
    error = avahi_entry_group_add_service_strlst(
        group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, "TestX", "_qotd._tcp", NULL, NULL, 123, txt);
    ...
    st = avahi_service_type_browser_new(
        avahi, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, NULL, 0, avahi_service_type_browser_callback, (char *)"omghai3u");
    if (st == NULL)
    ...
    sb = avahi_service_browser_new(
        avahi, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, "_http._tcp", NULL, 0, avahi_service_browser_callback, (char *)"omghai3u");
    if (sb == NULL)
        printf("Failed to create service browser object\n");
    else
        printf("Successfully created service browser %p\n", (void *)sb);

    hnr = avahi_host_name_resolver_new(avahi,
                                       AVAHI_IF_UNSPEC,
                                       AVAHI_PROTO_UNSPEC,
                                       "ecstasy.local",
                                       AVAHI_PROTO_UNSPEC,
                                       0,
                                       avahi_host_name_resolver_callback,
                                       (char *)"omghai4u");
    if (hnr == NULL)
    ...
    if (aar == NULL) {
        ...
        if ((error = avahi_entry_group_add_address(group2, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, "test-mdns.local.", aar)) <
            0) {
